...
import { importLegacyDealsSnapshot } from "./integrations/legacy-deals-importer";
import { DealRepository } from "./repositories/deal-repository";

type DealsCliCommand =
  | { command: "list"; dataDir: string; status: DealStatus | "all"; limit: number }
  | { command: "status"; dataDir: string }
  | { command: "import-legacy"; dataDir: string; source: string; sourceRef: string; apply: boolean };

const root = resolve(import.meta.dir, "../../..");

export function parseDealsCliArgs(args: string[]): DealsCliCommand {
  ...
  if (command === "list") {
    ...
    if (status !== "all" && !DEAL_STATUS_VALUES.includes(status as DealStatus)) fail("--status is invalid");
    const limit = Number(options.get("limit") ?? "50");
    if (!Number.isSafeInteger(limit) || limit < 1 || limit > 100) fail("--limit must be 1-100");
    return { command, dataDir, status: status as DealStatus | "all", limit };
  }
  if (command === "import-legacy") {
    ...
    return {
      ...
      sourceRef: options.get("source-ref") ?? "notice-ntfy:deals:d75fa2d",
      apply: options.get("apply") === "true",
    };
  }
  fail("Usage: deals <list|status|import-legacy> [options]");
}

export async function runDealsCli(command: DealsCliCommand): Promise<unknown> {
  if (command.command === "import-legacy") return importLegacyDealsSnapshot({
    sourcePath: command.source, sourceRef: command.sourceRef, dataDir: command.dataDir, apply: command.apply,
  });
  const repository = new DealRepository(command.dataDir);
  return command.command === "status"
    ? repository.summary()
    : { deals: repository.list(command.status, command.limit) };
}
...
if (import.meta.main) {
  try { console.log(JSON.stringify(await runDealsCli(parseDealsCliArgs(Bun.argv.slice(2))), null, 2)); }
  catch (error) {
