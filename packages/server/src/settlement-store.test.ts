...
test("persists workplaces and mission requests across store instances", async () => {
  ...
  const workspace = join(dataDir, "demo");
  await mkdir(workspace);
  const store = new SettlementStore(dataDir);
  const workplace = await store.addWorkplace("Demo", workspace);
  ...
  expect(restored.workplaces[0]).toMatchObject({ name: "Demo", path: workspace, policy: { version: 1, validation_commands: ["bun test"] } });
